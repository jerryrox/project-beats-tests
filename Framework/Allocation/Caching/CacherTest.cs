...
namespace PBFramework.Allocation.Caching.Tests
{
    public class CacherTest
    {
        ...
        public IEnumerator TestRequest()
        {
            ...
            Assert.IsNull(listener2.Output.Value);
            ...
            Assert.IsNotNull(listener2.Output.Value);
            Assert.IsTrue(cacher.IsCached("Key1"));
            Assert.IsTrue(cacher.IsCached("Key2"));
            Assert.IsFalse(cacher.IsCached("ASDF"));
            Assert.AreEqual("Key1", listener.Output.Value.Key);
            Assert.AreEqual("Key2", listener2.Output.Value.Key);
        }
        ...
        public IEnumerator TestRemove()
        {
            ...
            Assert.IsNotNull(listener2.Output.Value);
            Assert.IsNotNull(listener3.Output.Value);

            var value2 = listener2.Output.Value;
            var value3 = listener3.Output.Value;
        ...
        public IEnumerator TestDataLockRequest()
        {
            ...
            for (int i = 0; i < listeners.Length; i++)
            {
                if(i < listeners.Length - 1)
                    Assert.IsNull(listeners[i].Output.Value);
                else
                    Assert.IsNotNull(listeners[i].Output.Value);
            }
            cacher.Remove(listeners[listeners.Length - 1]);
            Assert.IsFalse(cacher.IsCached("aa"));
            Assert.IsTrue(listeners[listeners.Length - 1].Output.Value.IsDestroyed);
        }
